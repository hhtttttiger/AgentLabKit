...
import { streamConversationMessage } from '../api';
...
export function ProjectsPage() {
  ...
  async function send() {
    ...
    const userTurn: Turn = { turnId: `pending-${Date.now()}`, role: 'user', content: text };
    const assistantTurn: Turn = { turnId: `pending-assistant-${Date.now()}`, role: 'assistant', content: '' };
    setTurns((items) => [...items, userTurn, assistantTurn]);
    streamConversationMessage(conversation.conversationId, text, {
      onEvent: (event) => {
        if (event.type === 'error') { setTurns((items) => items.filter((item) => item.turnId !== assistantTurn.turnId)); setError(event.errorMessage ?? 'Execution failed'); return; }
        setTurns((items) => items.map((item) => item.turnId === assistantTurn.turnId ? { ...item, content: event.replyText ?? `${item.content}${event.delta ?? ''}`, runId: event.runId ?? item.runId } : item));
      },
      onError: (e) => { setTurns((items) => items.filter((item) => item.turnId !== assistantTurn.turnId)); setError(e.message); setBusy(false); },
      onComplete: () => { setBusy(false); void selectConversation(conversation); },
    });
  }
  return <div className="flex h-full min-h-0 flex-col bg-surface text-text">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[220px_260px_1fr]">
      ...
      <main className="flex min-h-0 flex-col p-6">{conversation ? <><div className="mb-4"><h2 className="text-lg font-semibold">{conversation.title}</h2><p className="text-xs text-text-muted">{project?.workspace}</p></div><div className="min-h-0 flex-1 space-y-4 overflow-auto">{turns.map((turn) => <div key={turn.turnId} className={turn.role === 'user' ? 'ml-12 rounded bg-surface-muted p-3' : 'mr-12 rounded border border-border p-3'}><div className="mb-1 text-xs font-semibold uppercase text-text-muted">{turn.role}</div><div className="whitespace-pre-wrap text-sm">{turn.content}</div>{turn.runId && <button type="button" className="mt-2 text-xs text-primary hover:underline" onClick={() => { window.location.href = `/runs/${turn.runId}`; }}>Run {turn.runId}</button>}</div>)}</div><div className="mt-4 flex gap-2"><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void send(); } }} placeholder="Continue the conversation…" className="min-h-12 flex-1 rounded border border-border bg-surface px-3 py-2 text-sm" /><button disabled={busy} onClick={() => void send()} className="rounded bg-primary px-4 py-2 text-sm text-white disabled:opacity-50">{busy ? 'Running…' : 'Send'}</button></div></> : <div className="flex h-full items-center justify-center text-sm text-text-muted">Choose a project and conversation to continue.</div>}{error && <div className="mt-3 text-sm text-danger">{error}</div>}</main>
    </div>
  </div>;
}
