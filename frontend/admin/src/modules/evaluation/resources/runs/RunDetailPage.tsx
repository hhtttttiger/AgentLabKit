  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">{runAgainAvailable && <button type="button" onClick={runAgain} disabled={triggerMutation.isPending} className="inline-flex items-center gap-2 bg-primary px-3 py-2 text-sm text-background disabled:opacity-50"><RotateCcw size={14} /> {triggerMutation.isPending ? 'Starting…' : 'Run Again'}</button>}{compareAvailable && <button type="button" onClick={() => navigate(`/evaluation/runs/compare?left=${encodeURIComponent(baseline!)}&right=${encodeURIComponent(run.id)}`)} className="inline-flex items-center gap-2 border border-border px-3 py-2 text-sm hover:bg-surface-raised"><GitCompare size={14} /> Compare with baseline #{baseline}</button>}{config?.targetType === 'agent' && <button type="button" onClick={() => navigate(`/agents/${encodeURIComponent(config.targetKey)}`)} className="inline-flex items-center gap-2 border border-border px-3 py-2 text-sm hover:bg-surface-raised"><ExternalLink size={14} /> Open Agent</button>}</div>
      </header>
