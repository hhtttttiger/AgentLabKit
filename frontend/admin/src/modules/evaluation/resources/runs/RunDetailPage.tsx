...
import type { CaseData, RetrievalEvidenceSummary, RunResultData } from '../../lib/contracts';
import { canCompare, canRunAgain, summarize, targetLabel } from './presentation';
...
export function RunDetailPage() {
  ...
  const runAgainAvailable = canRunAgain(run.status);
  const compareAvailable = canCompare(run.status, baseline);
  ...
  // Continuity: carry the evaluation identity into the Agent Builder so the
  // Improve leg (open → modify → publish → return) can find its way back.
  // Navigation context only — never a domain fact about the evaluation.
  const openAgent = config?.targetType === 'agent'
    ? () => navigate(`/agents/${encodeURIComponent(config.targetKey)}?returnEvaluation=${encodeURIComponent(run.id)}`)
    : null;
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="flex items-center gap-3"><h1 className="font-mono text-lg text-text">Evaluation Run #{run.id}</h1><Status status={run.status} /></div><div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-sm text-text-secondary"><span>{targetLabel(config?.targetType)}: <strong className="font-medium text-text">{config?.targetKey ?? '—'}</strong></span><span>Dataset: <strong className="font-medium text-text">{dataset?.name ?? config?.datasetId ?? '—'}</strong></span><span>Configuration: <strong className="font-medium text-text">{config?.name ?? run.configId}</strong></span></div></div>
        <div className="flex flex-wrap gap-2">{runAgainAvailable && <button type="button" onClick={runAgain} disabled={triggerMutation.isPending} className="inline-flex items-center gap-2 bg-primary px-3 py-2 text-sm text-background disabled:opacity-50"><RotateCcw size={14} /> {triggerMutation.isPending ? 'Starting…' : 'Run Again'}</button>}{compareAvailable && <button type="button" onClick={() => navigate(`/evaluation/runs/compare?left=${encodeURIComponent(baseline!)}&right=${encodeURIComponent(run.id)}`)} className="inline-flex items-center gap-2 border border-border px-3 py-2 text-sm hover:bg-surface-raised"><GitCompare size={14} /> Compare with baseline #{baseline}</button>}{openAgent && <button type="button" onClick={openAgent} className="inline-flex items-center gap-2 border border-border px-3 py-2 text-sm hover:bg-surface-raised"><ExternalLink size={14} /> Open Agent</button>}</div>
      </header>
      <section className="grid grid-cols-2 gap-3 md:grid-cols-6" aria-label="Evaluation summary">{[['Average Score', formatScore(summary.avgScore)], ['Passed', formatSummaryValue(summary.passed)], ['Failed', formatSummaryValue(summary.failed)], ['Unknown', formatSummaryValue(summary.unknown)], ['Errors', formatSummaryValue(summary.errors)], ['Total', formatSummaryValue(summary.total)]].map(([label, value]) => <div key={String(label)} className="border border-border bg-surface px-4 py-3"><div className="text-xs text-text-muted">{label}</div><div className="mt-1 text-lg font-semibold text-text">{value}</div></div>)}</section>
      ...
      <CaseDrawer result={selected} example={selected ? caseById.get(String(selected.caseId)) : undefined} agentKey={openAgent ? config?.targetKey : undefined} onClose={() => setSelected(null)} onAgent={() => openAgent?.()} onDataset={() => config?.datasetId && navigate(`/evaluation/dataset/${config.datasetId}`)} onRun={() => selected?.candidateRunId && navigate(`/runs/${encodeURIComponent(selected.candidateRunId)}`)} onTrace={() => selected?.candidateTraceId && navigate(`/traces/${encodeURIComponent(selected.candidateTraceId)}`)} />
    </div>
  );
}
...
function CaseDrawer({ result, example, agentKey, onClose, onAgent, onDataset, onRun, onTrace }: { result: RunResultData | null; example?: CaseData; agentKey?: string; onClose: () => void; onAgent: () => void; onDataset: () => void; onRun: () => void; onTrace: () => void }) {
  const { t } = useTranslation(['evaluation']);
  const evidence = result?.metricResults.find((metric) => metric.evidence)?.evidence ?? null;
  return <Modal open={!!result} title={result ? `Case #${result.caseId}` : 'Case'} onClose={onClose} footer={<div className="flex flex-wrap justify-between gap-2"><div className="flex items-center gap-3"><button type="button" onClick={onDataset} className="text-sm text-primary hover:underline">Open Dataset</button>{result?.candidateRunId && <button type="button" onClick={onRun} className="text-sm text-primary hover:underline">{t('runs.evidence.openRun')}</button>}{result?.candidateTraceId && <button type="button" onClick={onTrace} className="text-sm text-primary hover:underline">{t('runs.evidence.inspectTrace')}</button>}</div>{agentKey && <button type="button" onClick={onAgent} className="bg-primary px-3 py-2 text-sm text-background">Open Agent</button>}</div>}><div className="space-y-5 text-sm">{result && <><Detail label="Input" value={example?.inputText} /><Detail label="Expected Output" value={example?.expectedOutput} /><Detail label="Actual Output" value={result.actualOutput} /><div className="grid grid-cols-3 gap-3"><Detail label="Verdict" value={verdict(result)} /><Detail label="Overall Score" value={formatScore(result.overallScore)} /><Detail label="Duration" value={`${result.durationMs}ms`} /></div><RetrievalEvidence evidence={evidence} /><div><h3 className="mb-2 font-semibold text-text">Metrics</h3><div className="space-y-3">{result.metricResults.map((metric) => <div key={metric.metricName} className="border-t border-border pt-2"><div className="flex justify-between font-medium text-text"><span>{metric.metricName}</span><span>{formatScore(metric.score)} · {metric.passed === null ? '—' : metric.passed ? 'PASS' : 'FAIL'}</span></div><p className="mt-1 whitespace-pre-wrap text-text-secondary">{metric.reasoning ?? '—'}</p>{metric.score === null && metric.reason && <p className="mt-1 font-mono text-xs text-text-muted">{metric.reason}</p>}</div>)}</div></div>{result.errorMessage && <Detail label="Error" value={result.errorMessage} />}</>}</div></Modal>;
}
function RetrievalEvidence({ evidence }: { evidence: RetrievalEvidenceSummary | null }) {
  const { t } = useTranslation(['evaluation']);
  if (!evidence) return null;
  const label = evidence.availability === 'available'
    ? t('runs.evidence.available', { attempts: evidence.attempts, successful: evidence.successfulAttempts, contexts: evidence.contextsUsed })
